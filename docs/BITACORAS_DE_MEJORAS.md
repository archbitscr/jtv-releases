...
*   ✅ **Tarea 36d:** Hotkey overhaul — fix de edición de hotkeys (typing state bypass al main process). Spacebar asignado a Pin HUD (toggle show+pin). Flechas L/R reasignadas a channel up/down con +/- como secundarios. Ctrl+flechas L/R para cambio de fuente. UI de cards en grid 2 columnas con soporte dual-key. Botón editar muestra X para cancelar. Main process actualizado para forwarding de Space y Ctrl+Arrow.
