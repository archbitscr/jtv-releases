...
*   **Progreso 2026-07-04:**
    ...
    9.  ⏳ **Pendiente:** `.tnav-*`, `.player-*`, `.vod-*`/`.vod-details-*`, `.settings-*`, `.modal-*`, `.volosd-*`, `.corner-*`, `.tooltip-*`, `.search-*` — no auditados aún para fragmentación. Bloques `DEV-ONLY` interrumpiendo continuidad de Settings — no abordado.
    *   Cada paso validado con build + prueba visual en la app (Home, Live TV, grid VOD, Player Bar, Sidebar) antes de commitear. 7 commits incrementales en `master`.
